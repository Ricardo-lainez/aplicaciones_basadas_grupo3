...
def procesar_y_almacenar(directorio_archivos: str):
    """
    Procesa y almacena documentos en ChromaDB
    """
    try:
        # Leer los documentos del directorio
        reader = SimpleDirectoryReader(directorio_archivos, recursive=False)
        documents = reader.load_data()
        
        if not documents:
            return "No se encontraron documentos para procesar"
        ...
        return f"Procesados {len(documents)} documento(s) con éxito usando Groq."
